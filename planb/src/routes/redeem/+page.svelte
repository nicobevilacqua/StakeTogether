...
let balance;
  ...
  async function load() {
	...
	async function useMaxETHER() {
		// usa el maximo
		vaultBalance = await weth.balanceOf(vault.address);
		ethmountVal = formatEther(vaultBalance);
		updateethmountVal()
	}

	function updateethmountVal() {
		outputStake = parseEther(String(ethmountVal || '0'))
		if (Number(formatEther(outputStake)) > Number(formatEther(vaultBalance))) {
			outputStake = balance;
			ethmountVal = formatEther(vaultBalance);
		}
	}
...
async function redeem() {
	...
	invalid = true;
  try {
    const tx = await vault.redeemETH(parseEther(ethmountVal));
    await tx.wait(1);
...
<div class="flex justify-center flex-col mx-auto w-full text-center max-w-lg">
    <div class="border rounded-lg flex flex-row mx-auto mt-14">
      <a class="p-2 m-1 rounded-lg w-28 uppercase text-sm font-semibold text-black no-underline hover:no-underline  hover:bg-slate-100" href="./">Mint</a>
      <a class="p-2 m-1 rounded-lg w-28 uppercase text-sm font-semibold text-black no-underline hover:no-underline bg-slate-300" href="./redeem">Redeem</a>
    </div>
    <div class="rounded-3xl bg-white shadow-sm w-full mt-10 mx-auto flex flex-col px-5 py-4 z-10">
      <h1 class="uppercase text-black text-3xl">Redeem</h1>
      <div class="border rounded-lg p-2 flex flex-col">
        <div class="flex flex-row text-xs text-gray-700 justify-between items-center">
          {#if loaded}
            <div>vETH a destakear</div>
